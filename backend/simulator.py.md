"""
============================================================
SIMULATEUR ESP32 - PV MONITOR (AVEC COORDONNÉES GPS)
============================================================
Ce script simule un ESP32 avec caméra OV3660 envoyant des données
aléatoires au serveur backend, exactement comme le ferait un vrai ESP.
...
SERVER_IP = "10.50.81.222"      # IP de ton PC
SERVER_PORT = 8000               # Port du serveur
DEVICE_ID = "esp2"               # ID unique pour ce simulateur (doit exister dans la base)

# ============================================================
# COORDONNÉES GPS DU DEVICE (IMPORTANT !)
# ============================================================
# Ces coordonnées sont envoyées lors de l'enregistrement
# et stockées en base pour la récupération d'irradiance
...
DEVICE_NAME = "Simulateur ESP32" # Nom du device
        ...
        if response.status_code in [200, 400]:
            ...
            if "access_token" in data:
                ...
                if response.status_code == 200:
                    ...
                    # Mettre à jour les coordonnées via l'API admin
...
def update_device_coordinates():
    ...
    """
    try:
        # D'abord obtenir un token si pas déjà fait
        if not auth_token:
            return
...
# ============================================================
# SIMULATION DES CAPTEURS ANALOGIQUES
# ============================================================

def read_temperature():
    """Simule la lecture de température"""
    temperature = 25.0 + random.uniform(-10, 15)
    return round(temperature, 2)

def read_voltage():
    """Simule la lecture de tension"""
    voltage = 18.0 + random.uniform(-2, 3)
...
def read_current():
    ...
    if 6 <= hour <= 18:  # Journée
        ...
        base_current = random.uniform(0, 0.5)
    return round(base_current, 2)
"""
def read_irradiance():

    Simule la lecture d'irradiance
    ...
    if hour < 6 or hour > 20:  # Nuit
        ...
        irradiance = random.uniform(800, 1000)
        

    elif 9 <= hour <= 10 or 15 <= hour <= 16:  # Matin/après-midi       
irradiance = random.uniform(400, 700)
    else:  # Lever/coucher   
irradiance = random.uniform(50, 300)
    
    # Nuages aléatoires
    if random.random() < 0.3:
        
irradiance *= random.uniform(0.3, 0.8)
...
def send_to_server(image_data, voltage, current, temperature):
    ...
    boundary = "ESP32SimulatorBoundary"
...
#   text_part += f"--{boundary}\r\n"
#   text_part += 'Content-Disposition: form-data; name="irradiance"\r\n\r\n'

#   text_part += f"{irradiance:.1f}\r\n"
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                if 'ai_analysis' in data:
                    ai = data['ai_analysis']
                    print(f"   🤖 IA: {ai['soiling_level']:.1f}% - {ai['status']} (confiance {ai['confidence']*100:.0f}%)")
            except:
                pass
            return True
        elif response.status_code == 401:
            print("⚠️ Token invalide, réinitialisation...")
            auth_token = ""
            return False
        else:
...
def print_header():
    ...
    print("=" * 60)
    print("🔆 SIMULATEUR ESP32 - PV MONITOR (AVEC GPS)")
    print("=" * 60)
        ...
        while True:
            ...
            if current_time - last_send >= INTERVAL_SEC:
               ...
               # irradiance = read_irradiance()
                ...
                print(f"⚡ {voltage:.2f}V  {current:.2f}A  {power:.1f}W  ")   #{irradiance:.1f}W/m²
                ...
                # Envoi au serveur
                success = send_to_server(image_data, voltage, current,  temperature) #irradiance,
